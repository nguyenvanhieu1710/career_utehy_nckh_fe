...
import { MapPin, Briefcase, TrendingUp, ArrowLeft, Star, ExternalLink } from 'lucide-react';
...
function renderMarkdownRich(text: string) {
  return text.split(/(\*\*[^*]+\*\*|\*[^*]+\*)/g).flatMap((segment, index) => {
    ...
    if (segment.startsWith('*') && segment.endsWith('*')) {
      return (
        <em key={index} className='not-italic font-medium'>
          {segment.slice(1, -1)}
        </em>
      );
    }
...
function ScoreBar({ label, value }: { label: string; value: number }) {
  ...
  const color = percent >= 70 ? 'bg-green-500' : percent >= 40 ? 'bg-yellow-400' : 'bg-red-400';
...
function SuitableJobDetailContent() {
  ...
  const [job] = useState<JobDetailData | null>(() => {
    try {
      const cachedJob =
        typeof window !== 'undefined' ? sessionStorage.getItem('selected_job_detail') : null;
      return cachedJob ? (JSON.parse(cachedJob) as JobDetailData) : null;
    } catch (e) {
  ...
  if (!job) {
    return (
      <div className='min-h-screen flex flex-col items-center justify-center gap-4 text-gray-500'>
        <p className='text-lg font-medium'>Không tìm thấy thông tin công việc.</p>
        <button
          onClick={() => router.back()}
  ...
  const compatibilityColor = job.compatibility >= 50 ? 'text-green-600' : 'text-red-500';
  const compatibilityBg =
    job.compatibility >= 50 ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200';
  ...
  return (
    <div className='min-h-screen bg-gray-50'>
      <div className='max-w-3xl mx-auto px-4 py-8 space-y-6'>
        ...
        <div className='bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4'>
          <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4'>
            <div className='flex items-start gap-4'>
              <div className='w-20 h-20 rounded-2xl bg-gray-100 border flex items-center justify-center overflow-hidden shrink-0'>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={job.image_url || job.logo || '/default-job.png'}
                  alt={job.company}
            ...
            {job.url_source && (
              <a
                href={
                  job.url_source.startsWith('http') ? job.url_source : `https://${job.url_source}`
                }
                target='_blank'
        ...
        {(job.explanation ||
          (job.skill_improvement_suggestions && job.skill_improvement_suggestions.length > 0)) && (
          <div className='bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4'>
            <h3 className='text-lg font-bold text-indigo-600'>Đánh giá và gợi ý</h3>
            {job.explanation && (
              <div className='prose prose-sm text-gray-700 max-w-none whitespace-pre-wrap'>
                {renderMarkdownRich(job.explanation)}
              </div>
            )}
            {job.skill_improvement_suggestions && job.skill_improvement_suggestions.length > 0 && (
              <div className='space-y-3'>
                <h4 className='text-sm font-semibold text-gray-900'>Gợi ý nâng cao kỹ năng</h4>
                <ul className='list-disc list-inside text-gray-700 space-y-2'>
                  {job.skill_improvement_suggestions.map((suggestion, i) => (
        ...
        {job.scores &&
          (() => {
            const visibleScores = Object.entries(job.scores).filter(
              ([key, value]) => key in scoreLabels && (value as number) > 0
            );
            ...
            return (
              <div className='bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4'>
                <div className='flex items-center gap-2 text-indigo-600 mb-2'>
                  ...
                  <h3 className='text-lg font-bold'>Phân tích chi tiết độ phù hợp từ AI</h3>
                </div>
                <div className='space-y-4'>
                  {visibleScores.map(([key, value]) => (
                    <ScoreBar key={key} label={scoreLabels[key]} value={value as number} />
                  ))}
                </div>
              </div>
            );

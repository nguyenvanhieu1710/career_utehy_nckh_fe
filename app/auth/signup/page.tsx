...
export default function SignUpPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        sendVerifyEmail(signUpForm.email, signUpForm.password, signUpForm.fullname).then(res => {
            setSendSuccess(true)
            toast.success('Gửi email xác thực thành công', {
                description: 'Mở hòm thư của bạn để xác thực tài khoản.',
                duration: 3000,
            });
        }).catch((err) => {
            toast.error('Gửi email xác thực không thành công', {
                description: "",
                duration: 3000
            });
        }).finally(() => {
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50 relative">
            ...
            >
                ...
                <div className="md:w-1/2 flex flex-col justify-center p-6">
                    ...
                    {!sendSuccess ? <></> : <div>
                        <Button color="#3a7f51ff" value="Quay lại trang đăng ký" iconLeft={<ChevronLeft />} backgroundColor="#ffffffff" onClick={()=>{
                            setSendSuccess(false)
                        }}/>
                    </div>}
                    <form className="space-y-4" onSubmit={handleSubmit}>
                        {sendSuccess ? <>
                            <div className="flex flex-col items-center text-green-800">
                                <MailCheck color="#0C6A4E" size={100} strokeWidth={1} />
                                <div>
                                    Gửi thư xác thực thành công
                                </div>
                                <div>
                                    Mở hòm thư của bạn và nhấn xác thực
                                </div>
                                <div>
                                    <Button value="Gửi lại" iconLeft={<RotateCcw />} backgroundColor="#e8980eff" />
                                </div>
                            </div>
                        </> : <>
                            <div>

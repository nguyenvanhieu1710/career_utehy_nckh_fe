    ...
    cvAPI
      ...
      .then((res) => {
        ...
        if (cv.sections == 'NONE') {
          ...
          secs = Array.isArray(parsed) ? parsed : DEFAULT_SECTIONS_VI;
        }
        const parsedBg = JSON.parse(cv.design_data || '[]');
        setBackgroundElements(Array.isArray(parsedBg) ? parsedBg : []);
        setSections(secs);
      })
      .catch(() => {});
  }, [cv_id]);
    ...
    <div className='flex text-gray-600 bg-white h-screen relative'>
      <CVToolBox
        ...
        onSectionLocationChange={(data) => {
          if (data.field == 'x') {
            setSections((prev) =>
              prev.map((sec) => (sec.id === data.id ? { ...sec, x: data.value } : sec))
            );
          } else if (data.field == 'y') {
            setSections((prev) =>
              prev.map((sec) => (sec.id === data.id ? { ...sec, y: data.value } : sec))
            );
          }
        }}
      />
      <div className='flex-10 flex text-gray-600 bg-white h-screen'>
        <CVCanvas
          onSectionResize={(data) => {
            ...
            setSections((prev) =>
              prev.map((sec) => (sec.id === data.id ? { ...sec, size: newSize } : sec))
            );
          }}
          cv_id={cv_id as string}
          projectName={projectName}
          ...
          onSectionDrag={(data) => {
            setSections((prev) =>
              prev.map((sec) => (sec.id === data.id ? { ...sec, x: data.x, y: data.y } : sec))
            );
          }}
          isSavable
      ...
      <div className='absolute top-2 right-4 zindex-10'>
        <Button
          ...
          iconLeft={
            patternSideExtend ? (
              <PanelRightClose color='#ffffffff' fill='#0C6A4E' />
            ) : (
              <PanelRightOpen color='#ffffff' fill='#0C6A4E' />
            )
          }
      ...
      <div
        className={` ${patternSideExtend ? 'flex-3' : 'flex-0'} p-2 h-screen flex flex-col gap-5 items-center overflow-y-auto overflow-hidden transition-all border-l border-gray-200`}
      >
          ...
          .map((cv) => {
            ...
            return (
              ...
              >
                ...
                <h3 className='font-medium text-gray-500 text-sm text-ellipsis w-40'>{cv.name}</h3>
                <span className='text-[10px] font-light text-gray-500'>
                  {' '}
                  Cập nhật cuối: {new Date(cv?.updated_at).toLocaleString()}
                </span>
              </div>
            );
          })}

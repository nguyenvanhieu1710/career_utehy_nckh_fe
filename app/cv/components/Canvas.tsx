...
export const generatePDFFromState = (state: CVState): void => {
  ...
  const drawItem = (
    ...
    item.children?.forEach((c) => {
      curY = drawItem(c, x, curY, maxWidth, depth + 1);
    });
    return curY;
  };
  ...
  ctx.save();
  ctx.beginPath();
  ctx.rect(0, 0, SIDEBAR_W, A4_HEIGHT);
  ctx.clip();
  ...
  for (let i = -A4_HEIGHT; i < A4_WIDTH + A4_HEIGHT; i += 36) {
    ctx.beginPath();
    ctx.moveTo(i, 0);
    ctx.lineTo(i + A4_HEIGHT, A4_HEIGHT);
    ctx.stroke();
  }
  ctx.restore();
  ...
  for (let row = 0; row < 4; row++)
    for (let col = 0; col < 5; col++) {
      ctx.beginPath();
      ctx.arc(18 + col * 14, A4_HEIGHT - 60 + row * 12, 2, 0, Math.PI * 2);
      ctx.fill();
    }
  ...
  const drawSections = () => {
    ...
    state.sections.forEach((section) => {
      const sw =
        section.size?.width ??
        (section.id === 'contact' || section.id === 'language' || section.id === 'skills'
          ? 200
          : 500);
      const sh =
        section.size?.height ??
...
export default function CVCanvas({
  ...
  const getSnappedPosition = (idx: number, nx: number, ny: number) => {
    ...
    for (const [i, l] of sectionLayouts.entries()) {
      ...
      if (Math.abs(nx - lx) < SNAP_THRESHOLD) {
        sx = lx;
        guideX = lx;
      }
      if (Math.abs(nx - lx - lw) < SNAP_THRESHOLD) {
        sx = lx + lw;
        guideX = lx + lw;
      }
      if (Math.abs(nx + cur.width - lx - lw) < SNAP_THRESHOLD) {
        sx = lx + lw - cur.width;
        guideX = lx + lw;
      }
      if (Math.abs(nx + cur.width - lx) < SNAP_THRESHOLD) {
        sx = lx - cur.width;
        guideX = lx;
      }
      if (Math.abs(ny - ly) < SNAP_THRESHOLD) {
        sy = ly;
        guideY = ly;
      }
      if (Math.abs(ny - ly - lh) < SNAP_THRESHOLD) {
        sy = ly + lh;
        guideY = ly + lh;
      }
      if (Math.abs(ny + cur.height - ly - lh) < SNAP_THRESHOLD) {
        sy = ly + lh - cur.height;
        guideY = ly + lh;
      }
      if (Math.abs(ny + cur.height - ly) < SNAP_THRESHOLD) {
        sy = ly - cur.height;
        guideY = ly;
      }
    }
  ...
  const getSnappedEdges = (
    ...
    for (const [i, l] of sectionLayouts.entries()) {
      if (i === idx) continue;
      const { x: lx, y: ly, width: lw, height: lh } = l;
      if (Math.abs(edges.left - lx) < SNAP_THRESHOLD) {
        snapLeft = lx;
        guideX = lx;
      }
      if (Math.abs(edges.left - lx - lw) < SNAP_THRESHOLD) {
        snapLeft = lx + lw;
        guideX = lx + lw;
      }
      if (Math.abs(edges.right - lx) < SNAP_THRESHOLD) {
        snapRight = lx;
        guideX = lx;
      }
      if (Math.abs(edges.right - lx - lw) < SNAP_THRESHOLD) {
        snapRight = lx + lw;
        guideX = lx + lw;
      }
      if (Math.abs(edges.top - ly) < SNAP_THRESHOLD) {
        snapTop = ly;
        guideY = ly;
      }
      if (Math.abs(edges.top - ly - lh) < SNAP_THRESHOLD) {
        snapTop = ly + lh;
        guideY = ly + lh;
      }
      if (Math.abs(edges.bottom - ly) < SNAP_THRESHOLD) {
        snapBottom = ly;
        guideY = ly;
      }
      if (Math.abs(edges.bottom - ly - lh) < SNAP_THRESHOLD) {
        snapBottom = ly + lh;
        guideY = ly + lh;
      }
    }

...
export const generatePDFFromState = (state: CVState): void => {
  ...
  const drawItemPDF = (
    ...
    const tx = bx + 10;
    const avail = maxW - depth * 20 - 10;
    const words = item.text.split(' ');
    let line = '',
      lineY = y;
    for (let i = 0; i < words.length; i++) {
      const test = line + words[i] + ' ';
      if (ctx.measureText(test).width > avail && i > 0) {
    ...
    item.children?.forEach((c) => {
      cy2 = drawItemPDF(c, x, cy2, maxW, depth + 1);
    });
    return cy2;
  };
  ...
  const els = [...(Array.isArray(state.backgroundElements) ? state.backgroundElements : [])].sort(
  ...
  for (const el of els) {
    ...
    if (el.type === 'rect') {
      ...
      ctx.font = '28px serif';
      ctx.fillStyle = el.iconColor || '#333';
  ...
  const drawSections = () => {
    state.sections.forEach((sec) => {
      ...
      if (isSidebar) {
        ...
        ctx.textAlign = 'left';
        ctx.fillText(sec.title.toUpperCase(), sx + 18, sy + 15);
        let iy = sy + 34;
        sec.items.forEach((item) => {
          ...
          for (let w = 0; w < words.length; w++) {
            ...
            if (ctx.measureText(test).width > sw - 28 && w > 0) {
        ...
        ctx.fillRect(sx, sy + 22, sw, 1);
        ctx.fillStyle = state.primaryColor;
        ctx.fillRect(sx, sy + 22, 32, 2);
        let iy = sy + 42;
        sec.items.forEach((item) => {
          iy = drawItemPDF(item, sx + 10, iy, sw - 20);
        });
      }
      ctx.restore();
    });
  ...
  const doRender = (img?: HTMLImageElement) => {
    if (img) {
      ...
      ctx.drawImage(
        img,
        (-s.width * s.scale) / 2 + s.offsetX,
        (-s.height * s.scale) / 2 + s.offsetY,
        s.width * s.scale,
        s.height * s.scale
      );
      ctx.restore();
...
export default function CVCanvas({
  ...
  const [imageInternalDragState, setImageInternalDragState] = useState<{
    startX: number;
    startY: number;
  } | null>(null);
  ...
  useEffect(() => {
    for (const el of backgroundElements) {
      if (el.type === 'icon' && el.iconSrc && !imgCache.current.has(el.iconSrc)) {
        ...
        img.crossOrigin = 'anonymous';
        img.onload = () => {
          imgCache.current.set(el.iconSrc!, img);
  ...
  useEffect(() => {
    ...
    sections.forEach((s, i) => {
      const sw =
        s.size?.width ??
        (s.id === 'contact' || s.id === 'language' || s.id === 'skills' ? 200 : 500);
      const sh =
        s.size?.height ??
        (s.id === 'about' ? 170 : s.id === 'experience' ? 400 : s.id === 'education' ? 550 : 200);
      m.set(i, { id: s.id, x: s.x, y: s.y, width: sw, height: sh });
    });
    setSectionLayouts(m);
  }, [sections]);
  ...
  const drawA4Paper = (ctx: CanvasRenderingContext2D) => {
    ...
    ctx.fillRect(b.centerX, b.centerY, A4_WIDTH, A4_HEIGHT);
  ...
  const drawBackgroundElements = (ctx: CanvasRenderingContext2D) => {
    ...
    const sorted = [...(Array.isArray(backgroundElements) ? backgroundElements : [])].sort(
      (a, b2) => (a.zIndex || 0) - (b2.zIndex || 0)
    );
    ...
    for (const el of sorted) {
      ...
      if (el.type === 'rect') {
        ...
        if (el.iconSrc) {
          ...
          // Emoji / char fallback
          ctx.font = '28px serif';
          ctx.fillStyle = el.iconColor || primaryColor;
          ctx.textAlign = 'center';
          ctx.textBaseline = 'middle';
  ...
  const drawItem = (
    ...
    lines.forEach((ln, idx) => {
      ...
      const dy = y + idx * 20;
      if (isLast) {
        drawTextUnderline(ctx, ln, textX, dy, item.style?.underline);
        return;
      }
      const ws = ln.split(' ');
      if (ws.length <= 1) {
      ...
      let xx = textX;
      ws.forEach((w) => {
        drawTextUnderline(ctx, w, xx, dy, item.style?.underline);
        xx += ctx.measureText(w).width + spaceW + extra;
      });
    });
    if (isEditing) ctx.globalAlpha = 1;
    let curY = y + lines.length * 20 + 5;
    item.children?.forEach((child) => {
      curY = drawItem(ctx, child, x, curY, maxWidth, depth + 1);
    });
    return curY;
  };
  ...
  const drawRoundedImage = (
    ...
    if (img?.complete) {
      ...
      ctx.drawImage(
        img,
        (-state.width * state.scale) / 2 + state.offsetX,
        (-state.height * state.scale) / 2 + state.offsetY,
  ...
  const drawResizeHandles = (
    ...
    const pts = [
      { x: layout.x, y: layout.y, p: 'nw' },
      { x: layout.x + layout.width, y: layout.y, p: 'ne' },
      { x: layout.x, y: layout.y + layout.height, p: 'sw' },
      { x: layout.x + layout.width, y: layout.y + layout.height, p: 'se' },
      { x: layout.x + layout.width / 2, y: layout.y, p: 'n' },
      { x: layout.x + layout.width / 2, y: layout.y + layout.height, p: 's' },
      { x: layout.x, y: layout.y + layout.height / 2, p: 'w' },
      { x: layout.x + layout.width, y: layout.y + layout.height / 2, p: 'e' },
    ];
    pts.forEach(({ x, y, p }) => {
  ...
  const drawImageHandles = (
    ...
    const pts = [
      { x: state.x, y: state.y, p: 'nw' },
      { x: state.x + state.width, y: state.y, p: 'ne' },
      { x: state.x, y: state.y + state.height, p: 'sw' },
      { x: state.x + state.width, y: state.y + state.height, p: 'se' },
      { x: state.x + state.width / 2, y: state.y, p: 'n' },
      { x: state.x + state.width / 2, y: state.y + state.height, p: 's' },
      { x: state.x, y: state.y + state.height / 2, p: 'w' },
      { x: state.x + state.width, y: state.y + state.height / 2, p: 'e' },
    ];
    pts.forEach(({ x, y, p }) => {
  ...
  const drawCVContent = (ctx: CanvasRenderingContext2D) => {
    ...
    if (!isIcon && (hoveredImage || imageDragState || imageResizeState))
      drawImageHandles(ctx, imageState, cx, cy);
    ...
    const drawSidebarHeader = (title: string, sx: number, sy: number, width: number) => {
      ...
      ctx.beginPath();
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (ctx as any).roundRect(sx + 8, sy, width - 16, 24, 5);
      ctx.fill();
      ctx.fillStyle = 'rgba(255,255,255,0.95)';
      ctx.font = 'bold 11px Arial';
      ctx.textAlign = 'left';
    ...
    sections.forEach((section, index) => {
      ...
      ctx.save();
      ctx.beginPath();
      ctx.rect(sx, sy, layout.width, layout.height);
      ctx.clip();
      ...
      if (guideLines.x !== null) {
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(cx + guideLines.x, cy);
        ctx.lineTo(cx + guideLines.x, cy + A4_HEIGHT);
        ctx.stroke();
      }
      if (guideLines.y !== null) {
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(cx, cy + guideLines.y);
        ctx.lineTo(cx + A4_WIDTH, cy + guideLines.y);
        ctx.stroke();
      }
      ctx.restore();
  ...
  const canvasToScreen = (cx2: number, cy2: number) => {
    ...
    if (!b) return { x: 0, y: 0 };
    return {
      x: (b.centerX + cx2) * zoom + pan.x + rect.left,
      y: (b.centerY + cy2) * zoom + pan.y + rect.top,
    };
  };
  ...
  const getImageResizeHandle = (x: number, y: number, s: ImageState): ResizeHandle => {
    const pts = [
      { x: s.x, y: s.y, p: 'nw' as ResizeHandle },
      { x: s.x + s.width, y: s.y, p: 'ne' as ResizeHandle },
      { x: s.x, y: s.y + s.height, p: 'sw' as ResizeHandle },
      { x: s.x + s.width, y: s.y + s.height, p: 'se' as ResizeHandle },
      { x: s.x + s.width / 2, y: s.y, p: 'n' as ResizeHandle },
      { x: s.x + s.width / 2, y: s.y + s.height, p: 's' as ResizeHandle },
      { x: s.x, y: s.y + s.height / 2, p: 'w' as ResizeHandle },
      { x: s.x + s.width, y: s.y + s.height / 2, p: 'e' as ResizeHandle },
    ];
    for (const h of pts) if (Math.hypot(x - h.x, y - h.y) <= HANDLE_SIZE) return h.p;
    return null;
  };
  ...
  const findItemAtCanvasPoint = (sectionIndex: number, _clickX: number, clickY: number) => {
    ...
    const walk = (items: SectionItem[], depth: number, path: number[]): any => {
      for (let i = 0; i < items.length; i++) {
        ...
        for (let w = 0; w < words.length; w++) {
          const test = line + words[w] + ' ';
          if (mc.measureText(test).width > available && w > 0) {
            line = words[w] + ' ';
            lineCount++;
          } else {
            line = test;
        ...
        if (clickY >= curY - 15 && clickY <= curY + itemH - 5) {
          return {
            ...
            text: item.text,
            canvasItemY: curY,
            canvasItemX: textX - 10,
            canvasWidth: available + 10,
          };
        }
        curY += itemH;
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (imageInternalDragState && isEditingImageInside) {
      const dx = pos.x - imageInternalDragState.startX,
        dy = pos.y - imageInternalDragState.startY;
      setImageState((p) => ({ ...p, offsetX: p.offsetX + dx, offsetY: p.offsetY + dy }));
      setImageInternalDragState({ startX: pos.x, startY: pos.y });
      return;
    }
    if (imageResizeState) {
      const dx = pos.x - imageResizeState.startX,
        dy = pos.y - imageResizeState.startY;
      const s = { ...imageResizeState.startState };
      switch (imageResizeState.handle) {
        case 'se':
          s.width = Math.max(60, s.width + dx);
          s.height = Math.max(60, s.height + dy);
          break;
        case 'sw':
          s.x += dx;
          s.width = Math.max(60, s.width - dx);
      ...
      setImageState((p) => ({
        ...p,
        x: pos.x - imageDragState.offsetX,
        y: pos.y - imageDragState.offsetY,
      }));
    } else if (resizeState) {
      const dx = pos.x - resizeState.startX,
        dy = pos.y - resizeState.startY;
      const nl = { ...resizeState.startLayout };
      switch (resizeState.handle) {
        case 'e':
          nl.width = Math.max(MIN_SECTION_WIDTH, nl.width + dx);
          break;
        case 'w':
          nl.x += dx;
          nl.width = Math.max(MIN_SECTION_WIDTH, nl.width - dx);
          break;
        case 's':
          nl.height = Math.max(MIN_SECTION_HEIGHT, nl.height + dy);
          break;
        case 'n':
          nl.y += dy;
          nl.height = Math.max(MIN_SECTION_HEIGHT, nl.height - dy);
      ...
      const snapped = getSnappedEdges(resizeState.sectionIndex, {
        left: nl.x,
        right: nl.x + nl.width,
        top: nl.y,
        bottom: nl.y + nl.height,
      });
      ...
      if (resizeState.handle?.includes('w') && snapped.snapLeft != null) {
        nl.x = snapped.snapLeft;
        nl.width = sr - snapped.snapLeft;
      }
      if (resizeState.handle?.includes('e') && snapped.snapRight != null) {
        nl.width = snapped.snapRight - nl.x;
      }
      if (resizeState.handle?.includes('n') && snapped.snapTop != null) {
        nl.y = snapped.snapTop;
        nl.height = sb - snapped.snapTop;
      }
      if (resizeState.handle?.includes('s') && snapped.snapBottom != null) {
        nl.height = snapped.snapBottom - nl.y;
      }
      const newLayouts = new Map(sectionLayouts);
      newLayouts.set(resizeState.sectionIndex, nl);
      ...
      const tryX = pos.x - dragSection.offsetX,
        tryY = pos.y - dragSection.offsetY;
      ...
      if (layout) {
        const nl = new Map(sectionLayouts);
        nl.set(dragSection.sectionIndex, { ...layout, x: snapped.x, y: snapped.y });
        setSectionLayouts(nl);
  ...
  const handleWheel = (e: React.WheelEvent<HTMLCanvasElement>) => {
    ...
    if (isEditingImageInside && hoveredImage) {
      ...
      setImageState((p) => ({
        ...p,
        scale: Math.max(0.5, Math.min(3, p.scale + (e.deltaY > 0 ? -0.1 : 0.1))),
      }));
      return;
    }
    e.preventDefault();
  ...
  const handleKeyDown = (e: KeyboardEvent) => {
    ...
    if (hoveredImage && e.key.toLowerCase() === 'r') {
      e.preventDefault();
      setImageState((p) => ({ ...p, rotation: (p.rotation + 15) % 360 }));
    }
    if (hoveredImage && e.key.toLowerCase() === 'l') {
      e.preventDefault();
      setImageState((p) => ({ ...p, rotation: (p.rotation - 15 + 360) % 360 }));
    }
  };
  ...
  const getSnappedPosition = (idx: number, nx: number, ny: number) => {
    ...
    const cur = sectionLayouts.get(idx);
    if (!cur) return { x: nx, y: ny, guideX, guideY };
    for (const [i, l] of sectionLayouts.entries()) {
      if (i === idx) continue;
      if (Math.abs(nx - l.x) < SNAP_THRESHOLD) {
        sx = l.x;
        guideX = l.x;
      }
      if (Math.abs(nx - l.x - l.width) < SNAP_THRESHOLD) {
        sx = l.x + l.width;
        guideX = l.x + l.width;
      }
      if (Math.abs(nx + cur.width - l.x - l.width) < SNAP_THRESHOLD) {
        sx = l.x + l.width - cur.width;
        guideX = l.x + l.width;
      }
      if (Math.abs(nx + cur.width - l.x) < SNAP_THRESHOLD) {
        sx = l.x - cur.width;
        guideX = l.x;
      }
      if (Math.abs(ny - l.y) < SNAP_THRESHOLD) {
        sy = l.y;
        guideY = l.y;
      }
      if (Math.abs(ny - l.y - l.height) < SNAP_THRESHOLD) {
        sy = l.y + l.height;
        guideY = l.y + l.height;
      }
      if (Math.abs(ny + cur.height - l.y - l.height) < SNAP_THRESHOLD) {
        sy = l.y + l.height - cur.height;
        guideY = l.y + l.height;
      }
      if (Math.abs(ny + cur.height - l.y) < SNAP_THRESHOLD) {
        sy = l.y - cur.height;
        guideY = l.y;
      }
    }
  ...
  const getSnappedEdges = (
    ...
    let snapLeft: number | null = null,
      snapRight: number | null = null,
      snapTop: number | null = null,
      snapBottom: number | null = null;
    let guideX: number | null = null,
      guideY: number | null = null;
    for (const [i, l] of sectionLayouts.entries()) {
      if (i === idx) continue;
      if (Math.abs(edges.left - l.x) < SNAP_THRESHOLD) {
        snapLeft = l.x;
        guideX = l.x;
      }
      if (Math.abs(edges.left - l.x - l.width) < SNAP_THRESHOLD) {
        snapLeft = l.x + l.width;
        guideX = l.x + l.width;
      }
      if (Math.abs(edges.right - l.x) < SNAP_THRESHOLD) {
        snapRight = l.x;
        guideX = l.x;
      }
      if (Math.abs(edges.right - l.x - l.width) < SNAP_THRESHOLD) {
        snapRight = l.x + l.width;
        guideX = l.x + l.width;
      }
      if (Math.abs(edges.top - l.y) < SNAP_THRESHOLD) {
        snapTop = l.y;
        guideY = l.y;
      }
      if (Math.abs(edges.top - l.y - l.height) < SNAP_THRESHOLD) {
        snapTop = l.y + l.height;
        guideY = l.y + l.height;
      }
      if (Math.abs(edges.bottom - l.y) < SNAP_THRESHOLD) {
        snapBottom = l.y;
        guideY = l.y;
      }
      if (Math.abs(edges.bottom - l.y - l.height) < SNAP_THRESHOLD) {
        snapBottom = l.y + l.height;
        guideY = l.y + l.height;
      }
    }
    ...
    <div className={`${isIcon ? '' : 'flex-1'} relative bg-[#F9FAFB]`} ref={containerRef}>
      ...
      {editingOverlay && (
        <>
          ...
          <textarea
            ...
            onChange={(e) =>
              setEditingOverlay((prev) => (prev ? { ...prev, text: e.target.value } : null))
            }
            onKeyDown={(e) => {
      ...
      {!isIcon && (
        <div className='absolute bottom-6 right-6 flex flex-col gap-2 bg-white rounded-lg shadow-lg border border-gray-200 p-2'>
          <button
            ...
            className='p-2 hover:bg-gray-100 rounded transition-all text-gray-700'
          >
            <ZoomIn size={20} />
          </button>
      ...
      {!isIcon && hoveredSection !== null && !dragSection && !resizeState && !editingOverlay && (
        <div className='absolute top-6 right-6 bg-white rounded-lg shadow-lg border border-[#0C6A4E] px-4 py-2'>
          ...
          <div className='flex items-center gap-2 text-sm text-[#0C6A4E] mt-1'>
            <Maximize size={16} />
            <span className='font-medium'>Handles to resize</span>
          </div>

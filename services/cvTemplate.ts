...
export const cvTemplateAPI = {
  ...
  createTemplate: (data: TemplateCreate) =>
    api.post<{ status: string; data: CVTemplate; default_sections: string }>(
      "/cv-templates/create",
      data,
    ),

  // Cập nhật thiết kế (Lưu từ trang Canvas)
  updateTemplateDesign: (id: string, cvData: any) =>
    api.put<{ status: string; message: string }>(
      `/cv-templates/update-design/${id}`,
      cvData,
    ),

  // Cập nhật thông tin cơ bản (Tên, danh mục, trạng thái)
  updateTemplateInfo: (id: string, data: Partial<CVTemplate>) =>
    api.put<{ status: string; data: CVTemplate }>(
      `/cv-templates/update-info/${id}`,
      data,
    ),
